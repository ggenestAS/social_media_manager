...
- **Browse brands/assets/calendars**: `npm run browse` → http://127.0.0.1:4173 (read-only,
  zero-dependency Node server in `app/`; rescans `brands/` on every request).
...
- **Press reels** (`brands/albert-prep-agent`): `npm run html:to-mp4 -- <bundle>/source.html --out <bundle>/export`.
  The board declares `data-speed="1"`, `data-loop-ms`, `data-cta-ms`, `data-cover-ms`
  itself (generated by the inlined `reel-timeline.js`), so no flags are needed.
  The same board emits `data-audio-cues`; the exporter mixes the brand's samples
  (`brands/albert-prep-agent/assets/audio/`) automatically and needs an ffmpeg with
  libx264 + aac + loudnorm (Playwright's bundled ffmpeg is not enough).
