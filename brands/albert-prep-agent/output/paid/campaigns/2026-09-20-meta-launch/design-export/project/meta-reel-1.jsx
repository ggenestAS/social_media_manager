...
const W = 1080, H = 1920, PAD = 96, SAFE = { top: 250, bottom: 480, right: 160 };
...
function SafeOverlay() {
  const band = { position: 'absolute', background: 'rgba(255,60,60,0.22)', outline: '2px dashed rgba(255,80,80,0.9)', outlineOffset: -2, pointerEvents: 'none' };
  return (<>
    <div style={{ ...band, left: 0, top: 0, width: W, height: SAFE.top }} />
    <div style={{ ...band, left: 0, top: H - SAFE.bottom, width: W, height: SAFE.bottom }} />
    <div style={{ ...band, left: W - SAFE.right, top: SAFE.top, width: SAFE.right, height: H - SAFE.top - SAFE.bottom }} />
  </>);
}
function Piece({ accent, showLogo, variant, safe }) {
  ...
  const PANEL_Y = 800;
  const panelTop = MOTION.slide(T, c.Right, 0.55, H, PANEL_Y);
  const paperH = panelTop;
  // headline size compresses as the panel rises
  const hSize = animate({ from: R.headSize, to: R.headSizeSmall * 0.82, start: c.Right, end: c.Right + 0.55, ease: Easing.easeOutExpo })(T);
  // camera drift
  ...
  const rightSize = (R.rightSize || 176) * 0.85 - ((R.rightSize || 176) * 0.85 - 26) * rightU;
  ...
  const heroLift = animate({ from: 0, to: -12, start: c.CTA, end: c.CTA + 0.6, ease: Easing.easeOutCubic })(T);
  ...
  return (
    <div data-screen-label={`t=${Math.floor(T)}s`} style={{ position: 'absolute', inset: 0, background: INK, overflow: 'hidden', fontFamily: FONT, color: FOG }}>
      <div style={{ position: 'absolute', inset: 0, transform: `scale(${zoom})`, transformOrigin: '50% 45%' }}>
        {/* PAPER */}
        <div style={{ position: 'absolute', left: 0, top: 0, width: W, height: paperH, background: PAPER, color: '#111', clipPath: `inset(0 0 ${(1 - wipe) * 100}% 0)`, boxSizing: 'border-box', padding: `${SAFE.top + 20}px ${SAFE.right}px ${PAD - 16}px ${PAD}px`, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'hidden' }}>
        ...
        {/* INK PANEL */}
        <div style={{ position: 'absolute', left: 0, top: panelTop, width: W, height: H - PANEL_Y, background: INK, boxSizing: 'border-box', padding: `${PAD - 24}px ${SAFE.right}px ${SAFE.bottom + 24}px ${PAD}px`, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 -40px 120px rgba(0,0,0,0.6)' }}>
          <div>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: '0 0.3em', fontSize: rightSize, fontWeight: 500, letterSpacing: rightU > 0.5 ? '0.18em' : '-0.035em', textTransform: rightU > 0.5 ? 'uppercase' : 'none', color: rightU > 0.5 ? MUTE : FOG, lineHeight: 1.15, minHeight: 32 }}>
            ...
            <h1 style={{ margin: 0, marginTop: 44, fontSize: (R.heroSize || 100) * 0.86, fontWeight: 500, letterSpacing: '-0.035em', lineHeight: 1.0, transform: `translateY(${heroLift}px)` }}>
          ...
          <div style={{ ...ctaM, transformOrigin: 'left center' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 18, padding: '20px 36px', borderRadius: 16, background: FOG, color: INK, fontSize: 32, fontWeight: 600 }}>{R.cta} <span style={{ fontSize: 42, lineHeight: 1 }}>↑</span></span>
          </div>
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <CompositionStage width={W} height={H} scenes={window.OM_SCENES} playback={window.OM_PLAYBACK} bg="#1a1c1f">
        <Piece accent={t.accent} showLogo={t.showLogo} variant={t.variant} safe={t.safeZones} />
      </CompositionStage>
      <TweaksPanel>
        ...
        <TweakToggle label="Show Reels/TikTok safe zones" value={t.safeZones} onChange={(v) => setTweak('safeZones', v)} />
